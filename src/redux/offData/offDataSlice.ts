...
const fetchNewCodes = createAsyncThunk<
  FetchCodes,
  {
    campagne: string;
    state: number;
  }
>("offData/fetchNewCodes", async ({ campagne, state }, thunkAPI) => {
  const response = await fetch(
    `https://amathjourney.com/api/off-annotation/data/${campagne}?state=${state}&flag=false`
  );
...
const fetchOffProductData = createAsyncThunk<OFFRequest, any>(
  ...
  async (offRequest, thunkAPI) => {
    ...
    const response = await fetch(
      `https://fr.openfoodfacts.org/api/v0/product/${code}.json?fields=${requestedFields.join(
        ","
      )}`
    );
    // product_name, images
...
export const offDataSlice = createSlice({
  ...
  reducers: {
    ...
    addCodes: (state, action: PayloadAction<{ codes: string[] }>) => {
      state.codes = [
        ...state.codes,
        ...action.payload.codes.filter((code) => !state.seen.includes(code)),
      ];
    },
  },
  extraReducers: (builder) => {
    builder.addCase(fetchNewCodes.fulfilled, (state, { payload }) => {
      const newCodes = payload.result
        .map(({ code }) => code)
