...
const slicedCode = (code) => {
    if (code.length === 13) {
        return `${code.slice(0, 3)}/${code.slice(3, 6)}/${code.slice(6, 9)}/${code.slice(9, 13)}`
    }
    return code
}
...
export const getImageUrl = (code, imageId, coordinates_image_size) => {
    ...
    return `${OFF_IMAGE_URL}/${slicedCode(code)}/${imageId}${coordinates_image_size === "full" ? '' : `.${coordinates_image_size}`}.jpg`
}